<?php
$installFile='../tongdac/install.lock';
$sqlFile='./tdcms.php';
...
if (file_exists($installFile)) {
    echo '
        ...
        <body>
        您已经安装过本系统，如果需重新安装，请先删除站点Tongdac目录下的 install.lock 文件，再进行安装。
        </body>
        </html>';
        exit();
}
...
switch ($step) {
    ...
    case '4':
        ...
        if (setconfig($_POST,$configFile)) {
            $msgs.='<li><span class="correct_span">&radic;</span>成功写入数据库配置文件</li> ';
                ...
                // if ($_config['VERSION']=="" && $serverhost!="localhost" && $serverhost!="127.0.0.1") {
                //     $versions['VERSION']= "V2CN_".$serverhost."_".getip();
                //     setconfig($versions,'../config.php');
                // }
        }else{
            $msgs='提示：配置文件写入失败！';
            include_once ("s3.php");
    ...
    case '5':
        include_once ("s5.php");
        @file_put_contents($installFile, "TDCMS INATALL OK ...");
        exit();
        break;
...
function re_prefix($str){
  $strs=str_replace("td_", trim($_POST['DB_PREFIX']), $str);
  return $strs;
}
